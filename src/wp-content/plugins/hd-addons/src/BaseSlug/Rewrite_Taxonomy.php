...
class Rewrite_Taxonomy {
    ...
    public function term_link( $link, $term, $taxonomy ) {
        ...
        foreach ( $taxonomies as $type => $custom_tax ) {
            ...
            if ( $custom_tax->public &&
                ...
                // woocommerce
                if ( $custom_tax->name === 'product_cat' && Helper::isWoocommerceActive() ) {
    ...
    public function request( $query_vars ) {
        if ( isset( $query_vars['addons_category_redirect'] ) ) {
            $cat_link = user_trailingslashit( home_url( $query_vars['addons_category_redirect'] ), 'category' );
            Helper::redirect( $cat_link );
            exit;
        }
