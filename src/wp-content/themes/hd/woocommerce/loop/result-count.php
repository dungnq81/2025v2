...
<p class="woocommerce-result-count" role="alert" aria-relevant="all" <?php echo ( empty( $orderedby ) || 1 === intval( $total ) ) ? '' : 'data-is-sorted-by="true"'; ?>>
	...
	if ( 1 === (int) $total ) {
		...
		printf( _n( '%1$d sản phẩm', '%1$d sản phẩm', $total, TEXT_DOMAIN ) . $orderedby_placeholder, $total, esc_html( $orderedby ) );
	} else {
		...
		printf( _nx( '%1$d&ndash;%2$d of %3$d result', '%1$d&ndash;%2$d of %3$d results', $total, 'with first and last result', TEXT_DOMAIN ) . $orderedby_placeholder, $first, $last, $total, esc_html( $orderedby ) );
	}
