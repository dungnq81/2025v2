...
class RecentPostsWidget extends AbstractWidget {
    ...
    public function widget( $args, $instance ): void {
        ...
        $css_class  = ! empty( $instance['css_class'] ) ? Helper::escAttr( $instance['css_class'] ) : '';
        $uniqid     = esc_attr( uniqid( $this->widget_classname . '-', false ) );

        $r = Helper::queryByLatestPosts( 'post', $number, $limit_time );
        if ( ! $r ) {
            return;
        }
            ...
            <nav class="<?= $uniqid ?>" aria-label="<?php echo $aria_label; ?>">
                <ul class="recent-post">
                    ...
                    foreach ( $r->posts as $recent_post ) :
                        ...
                        $post_thumbnail  = Helper::postImageHTML( $recent_post->ID, 'medium', [ 'alt' => $attr_post_title ], true );
                            ...
                            <div class="cover-content">
                                ...
                                <?php if ( $show_date || $show_cat ) : ?>
                                    <div class="meta">
                                        ...
                                        <?php endif;
                                        echo $show_cat ? Helper::getPrimaryTerm( $recent_post ) : '';
                                        ?>
                                    </div>
                                <?php
                                endif;
                                echo $show_desc ? Helper::loopExcerpt( $recent_post ) : '';
                                ?>
                            </div>
                        </li>
                    <?php endforeach; ?>
