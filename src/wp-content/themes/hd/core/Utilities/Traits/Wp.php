...
trait Wp {
	...
	public static function FQNLoad( ?string $path, bool $require_path = false, bool $init_class = false, string $FQN = '\\', bool $is_widget = false ): void {
		...
		foreach ( $files as $file_path ) {
			...
			if ( self::isMaliciousFile( $file_path ) ) {
			...
			if ( $require_path ) {
				try {
					...
					self::errorLog( "Error requiring file $file_path: " . $e->getMessage() );
					continue;
				}
			}

			$filenameFQN = rtrim( $FQN, '\\' ) . '\\' . basename( $file_path, '.php' );
	...
	private static function isMaliciousFile( string $file_path ): bool {
		if ( ! $handle = fopen( $file_path, 'rb' ) ) {
			return false;
		}

		$content = fread( $handle, 8192 ); // 8KB
		fclose( $handle );

		return (bool) preg_match( '/\b(eval|assert|base64_decode|gzinflate|str_rot13|hex2bin)\s*\(/i', $content );
	}
	...
	public static function breadCrumbBanner( mixed $attachment_id, string $size = 'widescreen', int $cache_in_hours = 12 ): string {
		if ( ! empty( $attachment_id ) ) {
			return '<div class="breadcrumbs-bg dark:opacity-[0.7]">' . self::attachmentImageHTML( $attachment_id, $size, [
					'class'   => 'w-full block',
					'loading' => 'lazy'
				] ) . '</div>';
		}

		$breadcrumb_bg = self::getThemeMod( 'breadcrumb_bg_setting' );
		...
		return ! empty( $cached_id )
			? '<div class="breadcrumbs-bg dark:opacity-[0.7]">' . self::attachmentImageHTML( $cached_id, $size, [
				'class'   => 'w-full block',
				'loading' => 'lazy'
			] ) . '</div>'
			: '';
